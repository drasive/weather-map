<?php namespace DimitriVranken\weather_map\BusinessLogic;
      ...
      class ConditionsMapGenerator extends WeatherMapGenerator {
          ...
          public function generateMap($date) {
              $filePath = \DimitriVranken\weather_map\BusinessLogic\PathManager::getCachedWeatherMapFile($date, \DimitriVranken\weather_map\WeatherMapType::Conditions);
              if (!\DimitriVranken\weather_map\BusinessLogic\ConfigurationReader::getMapsCache() || !file_exists($filePath)) {
                  ...
                  // Generate map
                  $mapGenerator = new \DimitriVranken\weather_map\UserInterface\ConditionsMapGenerator($weatherData);
